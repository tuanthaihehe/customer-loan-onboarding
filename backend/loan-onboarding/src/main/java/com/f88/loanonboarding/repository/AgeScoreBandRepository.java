package com.f88.loanonboarding.repository;

import java.time.LocalDate;
import java.util.Optional;
import java.util.UUID;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;

import com.f88.loanonboarding.entity.AgeScoreBand;

public interface AgeScoreBandRepository extends JpaRepository<AgeScoreBand, UUID> {

    @Query(value = """
            SELECT *
            FROM age_score_band band
            WHERE band.rule_set_code = :ruleSetCode
              AND band.is_active = TRUE
              AND band.effective_from <= :asOfDate
              AND (band.effective_to IS NULL OR band.effective_to >= :asOfDate)
              AND :age >= band.min_age
              AND (band.max_age IS NULL OR :age < band.max_age)
            ORDER BY band.priority ASC
            limit 1
            """, nativeQuery = true)
    Optional<AgeScoreBand> findActiveBand(
            @Param("ruleSetCode") String ruleSetCode,
            @Param("age") int age,
            @Param("asOfDate") LocalDate asOfDate
    );
}
