...
```text
Có backend chạy được, có Swagger, có API skeleton, có mock service, có rule skeleton tối giản, có tài liệu cho AI/DEV đọc.
```
...
| Mock-first | Không bị chặn bởi ERD/DB |
...
| Mock data provider đã tách | Service sạch hơn |
...
| Mock data provider | Đã tách thành package `mock` |
| Rule registry | Chuyển thành `Not now`, chưa cần cho demo |
| Scope | Chốt lại chỉ demo Flow 1 |
| Submit for approval | Đã bổ sung endpoint/response mock |
